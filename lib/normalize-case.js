...
function sentenceCase(text) {
    ...
    // - w is first child
    // - w's first non-WhiteSpace predecessor is a BigPunctuator, OR is a WordNode
    //   whose last child is a BigPunctuator
    sentences.forEach(s => {
        const nodes = s.children;
        let capNext = false;
        nodes.forEach((n, i) => {
            const type = n.type;
            if (isBigPunctuator(n) || (type === "WordNode" && isBigPunctuator(n.children[n.children.length - 1]))) {
                capNext = true;
                return;
            } else if (type !== "WordNode") {
                return;
            }
            ...
            // Capitalize first TextNode
            Array.prototype.some.call(n.children, child => {
